...
%-----------------------------------------------------------------------
% Job saved on 15-Jul-2024 15:06:09 by cfg_util (rev $Rev: 8183 $)
% spm SPM - SPM12 (dev)
% cfg_basicio BasicIO - Unknown
%-----------------------------------------------------------------------

root='/Users/oj/Desktop/pre_BIDS/BIDS_RBD'
...
for i = 1:numel(sub)
    ...
    anat = spm_select('FPList', anat_dir, '^sub-*.*_acq-Sagittal3DT1GRE_rec-BRAINMRINONCONTRASTDIFFUSION_T1w.nii$') % this will return the full path (FP) to the T1 file from the anat directory

    %find and select the functional data
    func = spm_select('ExtFPList', func_dir, '^sub-*.*_task-BRAINMRINONCONTRASTDIFFUSION_acq-AxialfMRIrest_bold.nii$', NaN) % this will give the full path to the task data, NaN will ensure you are loading all volumes present (i.e. consider the 4D file as a whole)

    cd(func_dir) % move into the subject specific folder containing the functional data

    matlabbatch{1}.spm.spatial.realign.estwrite.data = {{'/Users/oj/Desktop/pre_BIDS/BIDS_RBD/sub-01/func/sub-01_task-BRAINMRINONCONTRASTDIFFUSION_acq-AxialfMRIrest_bold.nii,1'}};
    ...
    matlabbatch{3}.spm.spatial.coreg.estimate.source = {'/Users/oj/Desktop/pre_BIDS/BIDS_RBD/sub-01/anat/sub-01_acq-Sagittal3DT1GRE_rec-BRAINMRINONCONTRASTDIFFUSION_T1w.nii,1'};
